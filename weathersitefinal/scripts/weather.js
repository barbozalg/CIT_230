...
var arIco = [];
var arWnd = [];
var w1ObjURL = "//api.openweathermap.org/data/2.5/forecast?id=" + townID + "&units=imperial&appid=2bf68e13904ea713eda41eaaa1672a58";
...
w1Obj.onload = function() {
    ...
    for(var i=0; i<wInfo.cnt; i++){
        ...
        if(typeof arIco[tmpD.getDay()]!="object"){ arIco[tmpD.getDay()] = [[], []]; }
        arIco[tmpD.getDay()] = addTipo(arIco[tmpD.getDay()],wInfo.list[i].weather[0].icon);
    ...
    for(var i=0; i<low.length; i++){
        ...
        var desC = bestDes(arDes[ord[i]]);
        gObj("ic"+i).innerHTML = '<img src="//openweathermap.org/img/w/' + bestDes(arIco[ord[i]]) + '.png" alt="' + desC + '">';
        gObj("dd"+i).innerHTML = desC; // Description Large Table
